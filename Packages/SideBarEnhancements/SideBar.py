...
class SideBarNuevoArchivo(sublime_plugin.WindowCommand):
	def run(self, paths=[], name=""):
		window=sublime.active_window()
		view=window.active_view()
		view.run_command("nuevo_archivo", {"folder":paths[0]})

class SideBarAgregarRecurso(sublime_plugin.WindowCommand):
	def run(self, paths=[], name=""):
		window=sublime.active_window()
		view=window.active_view()
		view.run_command("agregar_recurso", {"folder":paths[0]})
...
class SideBarOpenAndRemoveCommand(sublime_plugin.WindowCommand):
	def run(self, paths=[]):
		import subprocess
		items = []
		...
		for item in SideBarSelection(paths).getSelectedItems():
			sublime.active_window().run_command("side_bar_open_in_new_window", {"paths": paths})
			sublime.active_window().run_command("side_bar_project_item_remove_folder", {"paths": paths})
			print(item.path())
