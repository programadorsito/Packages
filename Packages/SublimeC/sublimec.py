...
class ImportarLibreriaCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		view=sublime.active_window().active_view()
		carret=view.sel()[0].a
		funcion=view.substr(view.word(sublime.Region(carret, carret)))
		f=open(sublime.packages_path()+"/../../../ayuda/c/c.xml")
		lineas=f.readlines()
		f.close()
		libreria=""
		for l in lineas:
			ide=l[l.find("id='")+4:l.find("'", l.find("id='")+4)]
			if l.find("<libreria ")!=-1:
				libreria=ide
			if l.find("<funcion ")!=-1:
				if ide.strip()==funcion.strip():
					self.view.insert(edit, 0, "#include <%s.h>\n"%libreria)


class sublimeC(sublime_plugin.EventListener):
	def on_query_completions(self, view, prefix, locations):
		if not utils.get_language() in ["c++", "c"]:return
		funciones="""
